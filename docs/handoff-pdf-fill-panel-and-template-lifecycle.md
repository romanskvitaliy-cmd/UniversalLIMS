...
**З 2026-05-27:** після успішного збереження значень (верхній `Зберегти`, кнопка в панелі, `Ctrl+S`, автозбереження) клієнт **також** пробує зберегти макет, якщо є незбережені зміни на вкладці «Позиція» (`isLayoutDirty`). Окрема кнопка «Зберегти макет у шаблон» лишається для ручного збереження лише макету.
...
- [ ] Бібліотека текстів: різні теги (`f327_*` / `Food_*`) не змішують записи
- [ ] Після зсуву на «Позиція» + `Зберегти` → повідомлення «Макет також збережено в шаблон» (або warning, якщо layout save не вдався)
